...
\begin{lemma}{Inversion} \label{lem:inversion}
  Note that for this lemma, all cases aside from 1 apply only if \(\tau\) is not
  quantified, but there is a lemma that shows you can attain the same results
  even if \(\tau\) is quantified.
\mbox{}
\begin{enumerate}
...
There are a few caveats, most dealing with cases where there is not only one
possible typing rule that can get the corresponding conclusion.

The first is a convenience lemma. If the type \(\tau\) is quantified (e.g.
\(\tau = \forall \alpha. \tau'\)), then we may still reach the same conclusions
we would be able to reach if \(\tau\) were not (e.g. if \(\hastp \Gamma {e_0
  e_1} {\forall \alpha. \tau}\), we may still apply case 2). We may use case 1
of the lemma to obtain \(\hastp \Gamma e {\tau'[\alpha := p]}\) for a fresh
\(p\), repeating until there are no more prenex quantifiers. Then, we may
perform the desired inversion. Afterwards, we may use the quantifier
introduction rule with each of the variables (which remain fresh) to reintroduce
the quantifiers at prenex on each of the conclusions of the appropriate types.

\todo{maybe the above and below should be its own lemma}

The second is that the derivation could come from a forall elimination, not just
from the rule that gives the desired conclusion. In which case it would need to
be argued that the conclusion could still be the same. This is true, and we will
show it for a single case as an example. The proof generalizes to the remaining
cases.
...
types. By the convenience lemma proved above, we can conclude that \(\hastp
\Gamma {e_0} {\tau_1 \to (\forall \alpha. \alpha)}\) and \(\hastp \Gamma {e_1}
{\tau_1}\). It is known that \(\sigma_1 \to (\forall \alpha. \sigma_2) = \forall
\alpha. (\sigma_1 \to \sigma_2)\). This means that we can use the quantifier
elimination rule to conclude that \(\hastp \Gamma {e_0} {\tau_1 \to \tau}\),
thereby giving the two desired conclusions for \(\hastp \Gamma {e_0 e_1} \tau\).

Finally, the variant case can either come from a deault variant derivation or a
regular variant derivation. The latter follows immediately from the rules. The
